...
def compute_eer(target_scores, nontarget_scores,A07_A20,cm_scores):
    ...
    if A07_A20==True:
        keys = cm_scores[4].values
        scores = cm_scores['1_x'].values
        _, _, indices_dict = unique(keys)
        EERA07_A20 = {}
        for key, value in indices_dict.items():
            if key == "bonafide" or key ==  "A20":
                bona_cm = np.array([])
                spoof_cm = scores[value]
                far = compute_far(spoof_cm, thresholds[min_index])
                EERA07_A20[key] = (1-far)*100
            else:
                bona_cm = np.array([])
                spoof_cm = scores[value]
                far = compute_far(spoof_cm, thresholds[min_index])
                EERA07_A20[key] = (far)*100
        return eer, thresholds[min_index], EERA07_A20
    else:
